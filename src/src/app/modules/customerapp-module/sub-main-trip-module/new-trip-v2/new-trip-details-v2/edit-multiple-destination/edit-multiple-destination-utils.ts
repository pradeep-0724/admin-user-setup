...
export class MultipleDestinationFormManupulation {
  ...
  getDeFaultDestinationFrom(item:any) {
    ...
    return fb.group({
      point_type: [item.point_type||1],
      location: fb.group({
        name: [item.location?.name||'', [Validators.required]],
        lng: [item.location?.lng||''],
        lat: [item.location?.lat||''],
        alias:[item.location?.alias||'']
      }),
      checklist: [item.checklist||[]],
      time: [ moment(moment(new Date(item.time)).tz(localStorage.getItem('timezone')))|| moment(moment(new Date()).tz(localStorage.getItem('timezone'))).startOf('day')],
  ...
  buildDestinationFormArray(destinations = [], initialValues) {
    ...
    destinations.forEach((destination, index) => {
      ...
      initialValues.point.push(getBlankOption());
    })
  }
