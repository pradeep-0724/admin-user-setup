...
import { MultipleDestinationFormManupulation, getDistanceData, getEstimateDistance, getDHMS, scrollToRight, swapInitialValues, ToolTipInfo } from '../../new-trip-v2-utils/new-trip-v2-utils';
...
export class MultipleDestinationComponent implements OnInit, OnChanges,OnDestroy {
  ...
  @Input() disabledIndex: number = 0;
  @Input () isNameRequired:boolean;
  @Input() canDisplayShowInInvoice :  boolean = true;
  ...
  constructor(private _fb: FormBuilder, private mapDirectionsService: MapDirectionsService, private _phone_codes_flag_service: PhoneCodesFlagService,
    private _companyModuleService: CompanyModuleServices, private _commonService: CommonService) { }
    ...
    if (this.multipleDestinationForm.valid && this.selectedRoute) {
      if (destinations.length == this.routeDestinationsData.length) {
        for (let index = 0; index < destinations.length; index++) {
          ...
          if (isValidValue(routeDestination.halt_time) &&  !isEqual(routeDestination.halt_time, destinations[index].halt_time)) {
            this.isUpdateActive = true;
            return
          }
          ...
          if (routeDestination.show_in_invoice !=undefined && !isEqual(routeDestination.show_in_invoice, destinations[index].show_in_invoice)) {
            this.isUpdateActive = true;
            return
          }
          if (isValidValue(routeDestination.contact_name) && !isEqual(routeDestination.contact_name, destinations[index].contact_name)) {
            this.isUpdateActive = true;
            return
          }
          ...
          if (isValidValue(routeDestination.contact_no) && !isEqual(routeDestination.contact_no, destinations[index].contact_no)) { 
            this.isUpdateActive = true;
            return
          }
