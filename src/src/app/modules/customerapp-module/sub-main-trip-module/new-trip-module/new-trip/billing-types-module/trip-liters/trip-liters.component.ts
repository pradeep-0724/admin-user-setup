...
@Component({
  selector: 'app-trip-liters',
  templateUrl: './trip-liters.component.html',
  styleUrls: ['./trip-liters.component.scss']
})
export class TripLitersComponent implements OnInit {

  literForm: UntypedFormGroup;
  total = {
    liter: '0.00',
    freight: '0.00'
  }
  ...
  ngOnInit() {
    ...
    this.isFormValid.subscribe(valid => {
      if (!valid) {
        this.setAsTouched(this.literForm);
      }
    })
  }

  buildForm() {
    this.literForm = this._fb.group({
      liter: this._fb.array([])
    })
    if (this.isEdit) {
      this.addItem(this.editData);
      this.calculateTotal();
      const itemarray = this.literForm.controls['liter'] as UntypedFormArray;
  ...
  calculateTotal() {
    ...
    itemarray.controls.forEach(item => {
      ...
      this.total.liter = (Number(this.total.liter) + Number(item.get('charged_units').value)).toFixed(3);
    });
    this.checkIfDataContails();
  ...
  prepareRequest() {
    this.literForm.valueChanges.pipe(debounceTime(300)).subscribe(data => {
      let outPutData = {
        isFormValid: this.literForm.valid,
        allData: []
      }
      if (this.literForm.valid) {
        outPutData = {
          isFormValid: this.literForm.valid,
          allData: data['liter']
        }
