...
@Component({
  selector: 'app-trip-gallons',
  templateUrl: './trip-gallons.component.html',
  styleUrls: ['./trip-gallons.component.scss']
})
export class TripGallonsComponent implements OnInit {


  gallonForm: UntypedFormGroup;
  total = {
    gallon: '0.00',
    freight: '0.00'
  }
  ...
  ngOnInit() {
    ...
    this.isFormValid.subscribe(valid => {
      if (!valid) {
        this.setAsTouched(this.gallonForm);
      }
    })
  }

  buildForm() {
    this.gallonForm = this._fb.group({
      gallon: this._fb.array([])
    })
    if (this.isEdit) {
      this.addItem(this.editData);
      this.calculateTotal();
      const itemarray = this.gallonForm.controls['gallon'] as UntypedFormArray;
  ...
  calculateTotal() {
    ...
    itemarray.controls.forEach(item => {
      ...
      this.total.gallon = (Number(this.total.gallon) + Number(item.get('charged_units').value)).toFixed(3);
    });
    this.checkIfDataContails();
  }
  ...
  prepareRequest() {
    this.gallonForm.valueChanges.pipe(debounceTime(300)).subscribe(data => {
      let outPutData = {
        isFormValid: this.gallonForm.valid,
        allData: []
      }
      if (this.gallonForm.valid) {
        outPutData = {
          isFormValid: this.gallonForm.valid,
          allData: data['gallon']
        }
