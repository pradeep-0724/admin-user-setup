...
@Component({
  selector: 'app-trip-kms',
  templateUrl: './trip-kms.component.html',
  styleUrls: ['./trip-kms.component.scss']
})
export class TripKmsComponent implements OnInit {

  kmsForm: UntypedFormGroup;
  total = {
    kms: '0.00',
    freight: '0.00'
  }
  ...
  ngOnInit() {
    ...
    this.isFormValid.subscribe(valid => {
      if (!valid) {
        this.setAsTouched(this.kmsForm);
      }
    })
  }

  buildForm() {
    this.kmsForm = this._fb.group({
      kms: this._fb.array([])
    })
    if (this.isEdit) {
      this.addItem(this.editData);
      this.calculateTotal();
      const itemarray = this.kmsForm.controls['kms'] as UntypedFormArray;
  ...
  calculateTotal() {
    ...
    itemarray.controls.forEach(item => {
      ...
      this.total.kms = (Number(this.total.kms) + Number(item.get('charged_units').value)).toFixed(3);
    });
    this.checkIfDataContails();
  }
  ...
  prepareRequest() {
    this.kmsForm.valueChanges.pipe(debounceTime(300)).subscribe(data => {
      let outPutData = {
        isFormValid: this.kmsForm.valid,
        allData: []
      }
      if (this.kmsForm.valid) {
        outPutData = {
          isFormValid: this.kmsForm.valid,
          allData: data['kms']
        }
