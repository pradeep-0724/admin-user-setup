...
export class TripDaysComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.isFormValid.subscribe(valid => {
      if (!valid) {
        this.setAsTouched(this.daysForm);
      }
    })
  }

  buildForm() {
    this.daysForm = this._fb.group({
      days: this._fb.array([])
    })
    if (this.isEdit) {
      this.addItem(this.editData);
      this.calculateTotal();
      const itemarray = this.daysForm.controls['days'] as UntypedFormArray;
  ...
  calculateTotal() {
    ...
    itemarray.controls.forEach(item => {
      ...
      this.total.days = (Number(this.total.days) + Number(item.get('charged_units').value)).toFixed(3);
    });
    this.checkIfDataContails();
  }
  ...
  prepareRequest() {
    this.daysForm.valueChanges.pipe(debounceTime(300)).subscribe(data => {
      let outPutData = {
        isFormValid: this.daysForm.valid,
        allData: []
      }
