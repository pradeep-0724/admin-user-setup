...
@Component({
  selector: 'app-invoice-settelment',
...
export class InvoiceSettelmentComponent implements OnInit,OnDestroy,AfterViewChecked {
  ...
  tabSelectionList: Array<any> = [
    {
      label: "All Payment Received List",
      value: "3",
    },
    {
  ...
  buttonData: any = {
    name: 'Add Payment Received',
    permission: Permission.payments__settlement.toString().split(',')[0],
    url: this.prefixUrl + '/income/payments/invoice/add'
  };

  listUrl = '/income/payments/list/invoice';
  filterUrl = 'revenue/payment_settlement/filters/';
  isLoading = false;
  ...
  ngAfterViewChecked(): void {
    this._setHeight.setTableHeight2(['.calc-height'],'payment-received-table',0)
  }
  ...
  ngOnInit() {
    ...
    this._analytics.addEvent(this.analyticsType.IN,this.analyticsScreen.PAYMENTSETTLEMENT,this.screenType.LIST,"Navigated");
  ...
  getTripList(params) {
    ...
    this._paymentService.getInvoiceList(params).subscribe((data) => {
      const container = document.querySelector('.fuel-list-wrap');
      container.scrollTo(0,0)
      this.paymentReceivedList = data['result'].ps;
  ...
  onScrollGetTripList(params) {
    this.isLoading = true;
    this._paymentService.getInvoiceList(params).subscribe(data => {
      this.paymentReceivedList.push(...data['result'].ps);
      params.next_cursor = data['result'].next_cursor;
...
openListDetailsData(e) {
 ...
 openGothrough(){
  ...
  deletePaymentsettelment(payment_id) {
    this.apiHandler.handleRequest(this._paymentService.deleteInvoicePayment(payment_id), `${this.paymentReceivedNumber} deleted successfully!`).subscribe(
      {
        next: () => {
          this._analytics.addEvent(this.analyticsType.DELETED, this.analyticsScreen.PAYMENTSETTLEMENT)
          this.getTripList(this.listQueryParams)
        },
        error: (error) => {
  ...
  popupFunction(data, index: any = null) {
    this.paymentReceivedNumber = data.payment_number
  ...
  confirmButton(event) {
    if (event) {
      ...
      this.deletePaymentsettelment(id);
      this.listIndexData = {};
    }
  }
  onClickCancel(){
