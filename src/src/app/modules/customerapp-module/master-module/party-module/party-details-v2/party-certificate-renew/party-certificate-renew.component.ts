...
export class PartyCertificateRenewComponent implements OnInit {
  ...
  constructor(
    ...
    private _partyDetailsClientService: PartyDetailsClientService,private apiHandler: ApiHandlerService
  ) {
  }
  ...
  onOkButtonClick() {
    if (this.renewDocuments.valid) {      
      ...
      this.apiHandler.handleRequest(this._partyDetailsClientService.renewCertificate(this.popUpRenewDocuments.data['id'], this.renewDocuments.value),`${this.renewDocuments.value['name']} renewed successfully!`).subscribe(resp => {
