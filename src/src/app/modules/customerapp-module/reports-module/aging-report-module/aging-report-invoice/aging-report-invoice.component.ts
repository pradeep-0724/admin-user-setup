...
@Component({
  selector: 'app-aging-report-invoice',
  templateUrl: './aging-report-invoice.component.html',
  styleUrls: ['./aging-report-invoice.component.scss']
})
export class AgingReportInvoiceComponent implements OnInit {
  apiError = '';
  currency_type: any;
  screenName = "ageing_invoice/"
  storeData = [];
  prefixUrl = '';
  filterUrl = 'revenue/ageing_invoice/filters/'
  tabSelectionList: Array<DropDownType> = [
    {
      label: "All Invoice Aging Report",
      value: "0",
    },
    {
      label: "Future",
  ...
  getAgingReport(params) {
    this._agingReport.getAgingReport(this.screenName,params).subscribe(data => {      
      this.storeData = data['result'].invoices;
  ...
  routeToInvoice(id) {
    ...
    this._route.navigate([this.prefixUrl + "/income/invoice/list"],{queryParams})
  }
  ...
  onScrollAgingReport(params){
    this.isLoading = true;
    this._agingReport.getAgingReport(this.screenName,params).subscribe(data => {
      this.storeData.push(...data['result'].invoices)
      params.next_cursor = data['result'].next_cursor;
