...
@Component({
  selector: 'app-gst-payable',
  templateUrl: './gst-payable.component.html',
  styleUrls: ['./gst-payable.component.scss'],
  host: {
    "(window:click)": "clickOutToHide($event)"
  }
})
export class GstPayableComponent implements OnInit {
  ...
  ngOnInit() {
    this._analytics.addEvent(this.analyticsType.IN,this.analyticsScreen.GSTP,this.screenType.VIEW,"Navigated");
  ...
  getGstReceivable(dateParams) {
    this._gstService.getGstPayable(dateParams).subscribe((response: any) => {
