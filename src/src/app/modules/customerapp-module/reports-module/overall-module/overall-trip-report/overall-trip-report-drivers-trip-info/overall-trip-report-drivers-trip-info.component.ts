...
@Component({
  selector: 'app-overall-trip-report-drivers-trip-info',
  templateUrl: './overall-trip-report-drivers-trip-info.component.html',
  styleUrls: ['./overall-trip-report-drivers-trip-info.component.scss']
})
export class OverallTripReportDriversTripInfoComponent implements OnInit {
  queryParams = {
    start_date: '',
    end_date: '',
    filters: '' 
  ...
  ngOnInit(): void {
    ...
    this._setHeight.setTableHeight2([".calc-height",".cal-height-drivers-trips"],"overall-drivers-trips-list",115)
  ...
  getTripHead() {
    this._overallTripReportService.getOverallTripReportDriversHead(this.queryParams).subscribe(resp => {
  ...
  getTripList() {
    this._overallTripReportService.getOverallTripReportDriversList( this.queryParamsTripList).subscribe((data) => {      
      this.tripList = data['result'].dri_trips;
      this.tripHeader = data['result'].header;
      this.tripIds = data['result'].dri_ids;
  ...
  onScrollGetTripList(params) {
    this.isLoading = true;
    this._overallTripReportService.getOverallTripReportDriversList( params).subscribe(data => {
      this.tripList.push(...data['result'].dri_trips);
      this.tripHeader=data['result'].header
      this.tripIds.push(...data['result'].dri_ids);
